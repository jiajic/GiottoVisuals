...
#' \[**discrete palettes**\] pulls n colors from discrete palette. If the number of
#' colors requested is within the number of discrete colors, the values will be
#' pulled without modification starting from the first value. If more are requested
#' than exist then the palette will be made continuous\cr
#' @param n numeric. number of colors wanted
#' @param pal character. palette to use. Partial matching with ignored capitalization
...
getColors <- function(pal = 'viridis',
  ...
  if (pal == 'distinct') return(getDistinctColors(n = n))

  # check palettes
  pkg_to_use = NULL
  if (!is.null(src)) pkg_to_use = g_match_arg(src, names(pal_names))
  if (!is.null(pkg_to_use)) pal = g_match_arg(pal, pal_names[[pkg_to_use]])
  else {
    try_val = try(g_match_arg(pal, pal_names[['base']]), silent = TRUE)
    pkg_to_use = 'base'
    if(inherits(try_val, 'try-error')) {
      try_val = try(g_match_arg(pal, pal_names[['hcl']]), silent = TRUE)
      pkg_to_use = 'hcl'
    }
    if(inherits(try_val, 'try-error')) {
      try_val = try(g_match_arg(pal, pal_names[['RColorBrewer']]), silent = TRUE)
      pkg_to_use = 'RColorBrewer'
    }
    if(inherits(try_val, 'try-error')) {
      try_val = try(g_match_arg(pal, pal_names[['viridis']]), silent = TRUE)
      pkg_to_use = 'viridis'
    }
    if(inherits(try_val, 'try-error')) {
      try_val = try(g_match_arg(pal, pal_names[['rcartocolor']]), silent = TRUE)
      pkg_to_use = 'rcartocolor'
    }
    if(inherits(try_val, 'try-error')) {
      try_val = try(g_match_arg(pal, pal_names[['wesanderson']]), silent = TRUE)
      pkg_to_use = 'wesanderson'
    }
    if(inherits(try_val, 'try-error')) {
      try_val = try(g_match_arg(pal, pal_names[['ggsci']]), silent = TRUE)
      pkg_to_use = 'ggsci'
    }
    if(inherits(try_val, 'try-error')) {
    ...
    if(inherits(try_val, 'try-error')) {
      try_val = try(g_match_arg(pal, pal_names[['NineteenEightyR']]), silent = TRUE)
      pkg_to_use = 'NineteenEightyR'
    }
    if(inherits(try_val, 'try-error')) {
      try_val = try(g_match_arg(pal, pal_names[['palettetown']]), silent = TRUE)
...
get_base_colors = function(n, pal = 'rainbow') {
  return(
    switch(
      ...
      'grey' = grDevices::grey.colors(n = n, start = 0, end = 1, gamma = 1),
      'gray' = grDevices::gray.colors(n = n, start = 0, end = 1, gamma = 1)
    )
  )
}
...
get_continuous_colors = function(col, n, strategy) {
  ...
  if(n < length(col)) return(col[1:n])
  switch(strategy,
         'cutoff' = return(col),
...
pal_names = list(
  ...
  base = c(
    'rainbow', 'heat', 'terrain.colors', 'topo.colors', 'cm.colors', 'grey', 'gray'
  ),
  RColorBrewer = c(
  ...
  viridis = c(
    'viridis', 'magma', 'cividis', 'inferno', 'mako', 'plasma', 'rocket', 'turbo'
  ),
  wesanderson = c(
  ...
  ggsci = c(
    'aaas', 'cosmic', 'd3', 'flatui', 'frontiers', 'futurama', 'gsea', 'igv', 'jama',
  ...
  palettetown = c(
    'charizard', 'pidgeotto', 'tangela', 'porygon', 'quilava', 'sunkern', 'yanma',
    'surskit', 'plusle', 'carvanha'
  ),
  palr = c(
